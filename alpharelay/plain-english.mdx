...
When AlphaRelay cannot be installed on the controller, an Android Copilot device can create **Controller Livestream** RTMP settings for a controller-native publisher, or receive the controller's HDMI output through a compatible USB capture card. See [Controller Livestream and HDMI Capture](/alpharelay/controller-video-inputs).
