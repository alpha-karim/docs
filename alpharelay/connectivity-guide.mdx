...
- Confirm **Tablet IP (LAN)** is prepopulated with the discovered pilot tablet.
- Use the IPv4 address shown on the pilot tablet only if discovery is blocked or the prepopulated address is wrong.
...
These usually mean AlphaRelay has local mission data or footage that still needs to reach the cloud.
